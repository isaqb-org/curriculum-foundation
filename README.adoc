...
* **(upcoming) version 2027 (will become mandatory from April 1^st^ 2027)**
* Version 2025 (mandatory from April 1^st^ 2025)**
...
| for 2027, CRs are accepted until June 15th 2026
...
| 4 | FLWG decides internally, which changes to accept for the upcoming release.| | RD - 9M, July 1st 2026
...
EP need to incorporate this updated version into their examination process and/or toolset until the release date RD.| | RD - 3M, Jan 15th 2027
...
* **RD**: Release-Date (next: April 1st 2027)
...
Two build options are available: <<build-docker,Docker>> (no local Java or Gradle installation required) and <<build-gradle,Gradle>> (requires a local Java 21 installation).

Please note: Either way, you need to include the submodules "pdf-theme", "html-theme", "license-copyright", and "gradle-tools" in your checkout, otherwise the build will fail. You can do that with the following commands:
...
[[build-docker]]
==== Option 1: Build with Docker (Recommended for Local Builds)

Prerequisite: You need https://docs.docker.com/get-docker/[Docker] installed.
No Java or Gradle installation is required on your local machine.
...
* builds a Docker image containing JDK 21 (first run only, cached afterwards),
* mounts the current repository directory into the container,
* runs `./gradle-tools/gradlew buildDocs` inside the container,
* writes the resulting PDF and HTML files into the `build/` directory of your local checkout.

Gradle downloads and caches are stored in a named Docker volume (`gradle-cache`) so subsequent runs are faster.

If you prefer plain Docker without Compose, you can also run:

[source,shell]
----
docker build -t curriculum-foundation-builder .
docker run --rm -w /project -v "$(pwd):/project" -v gradle-cache:/root/.gradle curriculum-foundation-builder
----

NOTE: Omitting the `-v gradle-cache:/root/.gradle` volume mount is possible, but Gradle will re-download its dependencies on every run.

[[build-gradle]]
==== Option 2: Build with Gradle (Requires Java 21)
