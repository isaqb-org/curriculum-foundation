...
* Dependency Injection als eine mögliche Lösung für das Dependency-Inversion-Prinzip 
 ...
 Softwarearchitekt:innen können  diejenigen der folgenden Muster erklären und Beispiele dafür liefern, die für die Kategorien von Systemen relevant sind, an denen sie selbst arbeiten (R3):
...
Software architects can explain and provide examples for those of the following patterns that are relevant for the categories of systems they build (R3):
